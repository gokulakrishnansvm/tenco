package com.tenco.navigation

object Routes {
    const val LANGUAGE = "language"
    const val LOGIN = "login"
    const val ROLE = "role"

    // Supplier
    const val SUPPLIER_HOME = "supplier"
    const val SUPPLIER_DEALERS = "supplier/dealers"
    const val SUPPLIER_DEALERS_LIST = "supplier/dealers-list"
    const val SUPPLIER_VENDORS = "supplier/vendors"
    const val SUPPLIER_PRICING = "supplier/pricing"
    const val SUPPLIER_TRANSACTIONS = "supplier/transactions"
    const val SUPPLIER_REPORTS = "supplier/reports"
    const val SUPPLIER_COMPLAINTS = "supplier/complaints"
    const val SUPPLIER_INSIGHTS = "supplier/insights"
    const val SUPPLIER_INVENTORY = "supplier/inventory"
    const val SUPPLIER_SELL = "supplier/sell"
    const val SUPPLIER_ORDERS = "supplier/orders"
    const val SUPPLIER_CASH_APPROVALS = "supplier/cash-approvals"
    const val SUPPLIER_ADJUSTMENTS = "supplier/adjustments"
    const val SUPPLIER_ADVANCES = "supplier/advances"
    const val SUPPLIER_STOCK_BY_SOURCE = "supplier/stock-by-source"
    const val SUPPLIER_ORDER_DETAIL = "supplier/order/{orderId}"
    fun orderDetail(id: String) = "supplier/order/$id"
    const val SUPPLIER_VENDOR_DETAIL = "supplier/vendor/{vendorId}"
    const val SUPPLIER_DEALER_DETAIL = "supplier/dealer/{dealerId}"
    fun vendorDetail(id: String) = "supplier/vendor/$id"
    fun dealerDetail(id: String) = "supplier/dealer/$id"
    const val PROFILE = "profile"
    const val NOTIFICATIONS = "notifications"

    // Vendor
    const val VENDOR_HOME = "vendor"
    const val VENDOR_PAY = "vendor/pay"
    const val VENDOR_ORDERS = "vendor/orders"
    const val VENDOR_MY_ORDERS = "vendor/my-orders"
    const val VENDOR_COMPLAINTS = "vendor/complaints-list"
    const val VENDOR_COMPLAINT = "vendor/complaint"
    const val VENDOR_HISTORY = "vendor/history"
}
